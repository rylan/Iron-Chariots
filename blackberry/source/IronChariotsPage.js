...
enyo.kind({
	...
	launchInBrowser: function(){
		if(this.currenturl) {
			var agrs1 = new blackberry.invoke.BrowserArguments(this.currenturl);
			blackberry.invoke.invoke(blackberry.invoke.APP_BROWSER, args1);
		} else {
			var agrs2 = new blackberry.invoke.BrowserArguments('http://wiki.ironchariots.org');
			blackberry.invoke.invoke(blackberry.invoke.APP_BROWSER, args2);
		}	
	},
	...
	getNewPage: function( inURL, inSender ){
		...
		} else if(inURL.search("qrc:") > -1){//windows 
			this.pageHistory.push( this.$.pageInfo.getContent() );
			if (this.currenturl) {
				this.urlhistory.push(this.currenturl);
			}
			var newUrl = inURL.replace( "qrc:", this.ironUrl );
			this.currenturl = newUrl;
			this.$.ironchariotsPage.setUrl(newUrl);
			this.$.pageInfo.setContent("");
			this.$.ironchariotsPage.call();
		...
		} else if(inURL.search("local://") > -1){//windows 
			this.pageHistory.push( this.$.pageInfo.getContent() );
			if (this.currenturl) {
				this.urlhistory.push(this.currenturl);
			}
			var newUrl = inURL.replace( "local://", this.ironUrl );
			this.currenturl = newUrl;
			this.$.ironchariotsPage.setUrl(newUrl);
			this.$.pageInfo.setContent("");
			this.$.ironchariotsPage.call();
		...
		} else if(inSender == null & inURL.search(this.ironUrl) > -1)
		{
			...
			var agrs = new blackberry.invoke.BrowserArguments('inURL');
